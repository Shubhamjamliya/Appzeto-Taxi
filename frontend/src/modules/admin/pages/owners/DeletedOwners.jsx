...
import { ChevronDown, ChevronRight, FileSearch, Loader2, Menu, RotateCcw, Trash2 } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const BASE = `${globalThis.__LEGACY_BACKEND_ORIGIN__}/api/v1/admin`;

const getOwnerName = (owner) => owner?.name || owner?.user_id?.name || owner?.company_name || '-';
const getOwnerEmail = (owner) => owner?.email || owner?.user_id?.email || '-';
const getOwnerMobile = (owner) => owner?.mobile || owner?.user_id?.mobile || '-';
...
const DeletedOwners = () => {
  ...
  const showingTo = totalEntries === 0 ? 0 : Math.min(showingFrom + pagedOwners.length - 1, totalEntries);

  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-950">
      <div className="mb-6 flex items-center justify-between border-b border-gray-200 bg-white px-5 py-3">
        <h1 className="text-xl font-bold uppercase tracking-wide text-slate-700">Manage Deleted Owners</h1>
        <div className="flex items-center gap-2 text-sm text-slate-400">
          ...
          <span>Manage Deleted Owners</span>
        </div>
      </div>
                  ...
                  {isLoading ? (
                    <tr>
                      <td colSpan="4" className="px-3 py-24 text-center">
                        <div className="flex flex-col items-center gap-4 text-slate-400">
                          ...
                          <p className="text-sm font-semibold">Loading deleted owners...</p>
                        </div>
                      </td>
                    </tr>
                  ) : pagedOwners.length === 0 ? (
                    <tr>
                      <td colSpan="4" className="border-b border-gray-200 px-3 py-12 text-center">
                    ...
                    pagedOwners.map((owner) => (
                      <tr key={owner._id} className="bg-white transition-colors hover:bg-gray-50">
                        <td className="px-3 py-5 text-sm text-gray-950">{getOwnerName(owner)}</td>
                        <td className="px-3 py-5 text-sm text-gray-950">{getOwnerEmail(owner)}</td>
                        <td className="px-3 py-5 text-sm text-gray-950">{getOwnerMobile(owner)}</td>
