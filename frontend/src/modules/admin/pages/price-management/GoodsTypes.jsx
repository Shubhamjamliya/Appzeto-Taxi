        ...
        {!isCreateOrEdit ? (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1.5 text-[11px] text-gray-400 font-medium tracking-tight">
                <span className="hover:text-gray-600 transition-colors cursor-pointer">Goods Type</span>
                <ChevronRight size={10} className="text-gray-300" />
                <span className="text-gray-800 font-bold">Listing</span>
              </div>
            </div>

            <div className="bg-white rounded-md border border-gray-100 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-50 flex items-center justify-between bg-white px-8">
                <div className="relative w-72">
                   <Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300" />
                   <input 
                      type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                      placeholder="Search..." 
                      className="w-full pl-10 pr-4 py-2 bg-gray-50/20 border border-gray-100 rounded text-xs focus:outline-none focus:border-indigo-300"
                   />
                </div>
                <button 
                  onClick={() => navigate('/admin/pricing/goods-types/create')}
                  className="flex items-center gap-2 px-8 py-2 bg-[#334155] text-white rounded text-xs font-black uppercase tracking-widest hover:bg-[#1E293B] shadow-sm transition-all active:scale-95"
                >
                ...
                <table className="w-full text-left">
                  <thead className="bg-[#FBFCFF]">
                    <tr className="border-b border-gray-50 text-[11px] text-gray-400 uppercase font-black tracking-widest">
                       ...
                       <th className="px-8 py-4">Compatible With</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {loading ? (
                      <tr><td colSpan="4" className="py-24 text-center text-slate-300 font-bold uppercase tracking-widest text-[10px] animate-pulse">Syncing recordset...</td></tr>
                    ) : filteredGoods.length === 0 ? (
                      <tr>
                         <td colSpan="4" className="py-24 text-center">
                            <div className="text-gray-200 mb-2"><Package size={40} className="mx-auto" /></div>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Empty Goods Catalog</p>
                         </td>
                      </tr>
                    ) : (
                      filteredGoods.map(item => (
                        <tr key={item._id || item.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="px-8 py-6 text-sm font-semibold text-gray-700">{item.name || item.goods_type_name}</td>
                          <td className="px-8 py-6 whitespace-nowrap">
                            <span className="px-3 py-1 bg-gray-50 text-gray-400 rounded text-[10px] font-bold uppercase border border-gray-100 italic">
                          ...
                          <td className="px-8 py-6 text-right pr-12">
                            <div className="flex items-center justify-end gap-2">
                               <button onClick={() => navigate(`/admin/pricing/goods-types/edit/${item._id || item.id}`)} className="p-2 text-gray-300 hover:text-indigo-600 transition-colors"><Edit2 size={16} /></button>
                               <button onClick={() => handleDelete(item._id || item.id)} className="p-2 text-gray-300 hover:text-rose-600 transition-colors"><Trash2 size={16} /></button>
                            </div>
                          </td>
                        </tr>
                      ))
          ...
          >
            ...
            <div className="flex items-center justify-between mb-8">
               ...
               <div className="flex items-center gap-1.5 text-[11px] text-gray-400 font-medium tracking-tight">
                  <span className="hover:text-gray-600 transition-colors cursor-pointer" onClick={() => navigate('/admin/pricing/goods-types')}>Goods Type</span>
                  <ChevronRight size={10} className="text-gray-300" />
                  <span className="text-gray-800 font-bold">{mode === 'edit' ? 'Edit' : 'Create'}</span>
               </div>
            </div>

            <div className="bg-white rounded border border-gray-100 shadow-sm overflow-hidden relative min-h-[500px]">
              {/* Language Tabs Area */}
              <div className="flex items-center border-b border-gray-100 bg-white">
                {languages.map(lang => (
                  <button
                    ...
                    className={`px-8 py-5 text-[12px] font-semibold transition-all relative ${activeTab === lang ? 'text-[#00BFA5]' : 'text-[#8E9FBC] hover:text-gray-700'}`}
                  >
                    {lang}
                    {activeTab === lang && (
              ...
              <form onSubmit={handleSave} className="p-10 lg:p-14 space-y-10 max-w-2xl">
                {/* Field: Name */}
                ...
                <div className="space-y-1.5">
                   ...
                   <div className="relative">
                      ...
                      >
                         <option value="">Select Transport Matrix</option>
                ...
                <div className="absolute right-10 top-[220px]">
                  <button type="button" className="w-12 h-12 bg-[#00BFA5] text-white rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-transform active:scale-95 group">
                    <div className="flex flex-col gap-1.5 items-center">
                       <div className="w-5 h-[2.5px] bg-white rounded-full group-hover:w-6 transition-all"></div>
                       <div className="w-5 h-[2px] bg-white/70 rounded-full"></div>
                       <div className="w-5 h-[1.5px] bg-white/40 rounded-full group-hover:w-4 transition-all"></div>
                    </div>
                  </button>
                </div>
                ...
                <div className="pt-10 flex justify-end">
                  <button 
                    ...
                    className="px-12 py-3 bg-[#4F5B71] text-white rounded text-sm font-bold shadow-md active:scale-95 transition-all flex items-center gap-3 hover:opacity-95"
                  >
