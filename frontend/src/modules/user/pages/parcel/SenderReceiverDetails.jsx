...
import { ArrowLeft, User, Phone, MapPin, AlertCircle, CheckCircle2, ChevronRight } from 'lucide-react';
...
const PhoneInput = ({ label, value, onChange, error, name, onClearError }) => (
  <div className="space-y-1">
    ...
    <AnimatePresence>
      {error && (
        <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-[11px] font-black text-red-500 ml-2 flex items-center gap-1">
  ...
  const handleProceed = () => {
    ...
    navigate('/parcel/searching', {
      state: {
        ...
        pickup,
        drop,
        pickupCoords: getCoords(pickup),
        dropCoords: getCoords(drop),
        senderName,
        senderMobile,
        receiverName,
        receiverMobile,
    ...
    <div className="min-h-screen bg-[#FDFDFD] max-w-lg mx-auto flex flex-col font-sans relative">
      ...
      <div className="flex-1 p-5 space-y-8 overflow-y-auto no-scrollbar pb-4">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-[32px] p-5 shadow-lg shadow-gray-100 border border-gray-50 space-y-4">
            <AddressField label="Pickup" value={pickup} onChange={(value) => { setPickup(value); clearError('pickup'); }} error={errors.pickup} suggestions={pickupSuggestions} onPickSuggestion={(value) => { setPickup(value); clearError('pickup'); }} />
            <AddressField label="Drop" value={drop} onChange={(value) => { setDrop(value); clearError('drop'); }} error={errors.drop} suggestions={dropSuggestions} onPickSuggestion={(value) => { setDrop(value); clearError('drop'); }} />
          </div>
        </div>
